...
def dynamic_write_results(prediction, confidence, num_classes, class_num, nms=True, nms_conf=0.4):
    prediction_bak = prediction.clone()
    dets = write_results(prediction.clone(), confidence, num_classes, class_num, nms, nms_conf)
    if isinstance(dets, int):
        return dets

    if dets.shape[0] > 100:
        nms_conf -= 0.05
        dets = write_results(prediction_bak.clone(), confidence, num_classes, class_num, nms,  nms_conf)

    return dets


def write_results(prediction, confidence, num_classes, class_num, nms=True, nms_conf=0.4):
