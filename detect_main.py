...
def detect_main(im_name, orig_img, det_model, pose_model, opt):
    ...
    for i in range(1):
        with torch.no_grad():
            ...
            for human in result['result']:
                ...
                r = (w ** 2 + h ** 2) ** 0.5 / d_ear
                # print(r)
                # cv2.circle(img, (int(xhead), int(yhead)), 3, (0, 0, 255), -1)
                # cv2.circle(img, (int(keypoint[3][0]), int(keypoint[3][1])), 3, (0, 255, 0), -1)
                # cv2.circle(img, (int(keypoint[4][0]), int(keypoint[4][1])), 3, (0, 255, 0), -1)
                # cv2.circle(img, (int(xfeet), int(yfeet)), 3, (255, 0, 0), -1)
    ...
    # if len(fall_res) == 0:
    # cv2.imwrite('re_1302/' + im_name[0:-4] + '_re.jpg', img)
    #    fall_res.insert(0,False)
    # else:
    #    fall_res.insert(0,True)
    return keypoint_res
